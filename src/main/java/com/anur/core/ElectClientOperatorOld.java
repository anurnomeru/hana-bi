...
//     * 开始一场选举
//     */
//    public void beginSelection(List<HanabiNode> HanabiNodeList, Votes votes) {
//        this.clientShutDownHooker = new ShutDownHooker(" ----------------- 终止连接其他选举节点！ ----------------- ");
//
//        /** 1、移除上个世代的所有定时任务 */
//
//        /** 2、建立与其他节点的连接，如果连接还未建立，则建立连接，如果已经建立，则不必再建立连接 */
//        HanabiNodeList.forEach(HanabiNode -> {
//            //            if (!HanabiNode.isLocalNode()) {
//            ElectClient electClient = new ElectClient(HanabiNode.getServerName(), HanabiNode.getHost(), HanabiNode.getElectionPort(),
...
//                                                .getChannel(HanabiNode.getServerName())) == null) {
//                    try {
//                        Thread.sleep(100);
